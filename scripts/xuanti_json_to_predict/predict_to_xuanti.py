...
with open(xuanti_predict_path, 'r') as r:
    with open('matched_topic_info_v5_with_entities_sty_0329.txt', 'w') as w:
        for i, tagged_line in enumerate(r):
            # if i > 10:
            #     break
            entities_info = json.loads(tagged_line)
            ...
            xuanti_json.update({'entities_with_sty': entities_info})

            w.write(json.dumps(xuanti_json, ensure_ascii=False) + '\n')
