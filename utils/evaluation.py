...
def model_eval(args, model, dataloader, eval=True, predict=True, tokenizer=None, eval_path=None, predict_path=None, output_suffix=""):
        ...
        for batch in dataloader:
            ...
            if eval:
                if len(labels.shape) == 1:
                    ...
                    # label = torch.max(labels, dim=1)[1].numpy().tolist()
                    label = []
    ...
    if eval:
        ...
        # eval_dict['acc'] = accuracy_score(all_label, all_predict)
        # eval_dict['micro_f1'] = f1_score(all_label, all_predict, average='micro')
        # eval_dict['macro_f1'] = f1_score(all_label, all_predict, average='macro')
